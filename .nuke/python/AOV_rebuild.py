...
def validate_duplicates(p, multiline_to_list):
    '''Check for duplicate AOVs within and across editable columns.
    Returns a list of error message strings, empty list if clean.'''
    def clean_input(knob_value):
        raw = multiline_to_list(knob_value)
        return [l for l in raw if ":" not in l and "---" not in l and l.strip()]

    columns = {
        'LIGHTS READ':         clean_input(p.aov_lg_read.value()),
        'MISC READ':           clean_input(p.aov_misc_read.value()),
    ...
    ## each frozenset defines one permitted overlap pair, an AOV appearing in exactly MATERIALS or UTILITIES columns will not be flagged
    ALLOWED_OVERLAP = {
        frozenset({'MISC READ', 'MATERIALS'}),
        frozenset({'MISC READ', 'UTILITIES'}),
    }
        ...
        for aov in aovs:
            if aov in seen and aov not in reported:
                errors.append(
                    ...
                    "Please amend. Duplicate AOVs will break the rebuild."
                    % (aov, col_name)
                )
                reported.add(aov)
            seen.add(aov)
...
def setup_breakout_panel(node=None):
    ...
    class BreakoutPanel(nukescripts.PythonPanel):
        def __init__(self, node):
            nukescripts.PythonPanel.__init__(self, "Breakout Lightgroups, Materials and Utilities")
            self.setMinimumSize(1800, 1100)
            self.node = node
            self._lg_read_user_edited = False

            ## column render engine
            self.addKnob(nuke.Text_Knob('', '<center><b>Render Engine</b><center>'))
            self.addKnob(nuke.Text_Knob("spacer", "", " " * 20))
            ...
            #self.render_engine.setValue('Generic')  ## render_engine default value
            ...
            #self.edit_regex.setFlag(nuke.STARTLINE)
            ...
            self.addKnob(nuke.Text_Knob('spacer_row', '', ' '))
            self.addKnob(nuke.Text_Knob('', '<center><b>AOV Management</b><center>'))
            self.addKnob(nuke.Text_Knob("spacer", "", " " * 20))

            ## row of multiline knobs
            self.aov_lg_read = nuke.Multiline_Eval_String_Knob('aov_lg_read', '', '')
            self.aov_lg_read.setEnabled(False)
            self.aov_lg_read.setTooltip("Lists LIGHT AOVs found in the render.\n\nThis column is read only to see light AOVs that match the regex naming convention.\n\nIf you cannot see light AOVs you wish to populate in the 'Lightgroup' breakout, please update...\n\n'Lightgroup Regex'\n\nand click\n\n'Update AOV Lists'\n\nto re-populate AOV Management.")

            self.aov_misc_read = nuke.Multiline_Eval_String_Knob('aov_misc_read', '', '')
            self.aov_misc_read.setEnabled(True)
            self.aov_misc_read.setTooltip("Lists MATERIALS, UTILITIES & OTHER AOVs found in the render.\n\nAOVs listed under OTHER READ: can be cut or copied to columns\n\nADD LIGHTING:\n\nADD MATERIALS: (AOVs listed under MATERIALS READ: do not need to be copied to ADD MATERIALS: to be rebuilt)\n\nADD UTILITIES: (AOVs listed under UTILITIES READ: do not need to be copied to ADD UTILITIES: to be rebuilt)\n\nOMIT: (as per user preference for breakout or omission).")
            ...
            for k in [self.aov_misc_read, self.additional_lighting, self.materials, self.utilities, self.omit_list]:
                k.clearFlag(nuke.STARTLINE)

            self.addKnob(self.aov_lg_read)
            self.addKnob(self.aov_misc_read)
            self.addKnob(self.additional_lighting)
            self.addKnob(self.materials)
            self.addKnob(self.utilities)
            self.addKnob(self.omit_list)
            ## toggle for LIGHTS READ column
            self.edit_lg_read = nuke.Boolean_Knob('edit_lg_read', 'Edit Lights Read')
            self.addKnob(nuke.Text_Knob("spacer", "", " " * 40))
            self.addKnob(nuke.Text_Knob("spacer", "", " " * 20))
            self.edit_lg_read.setTooltip("Enable this to edit the 'LIGHTS READ' column.")
            self.addKnob(self.edit_lg_read)
            self.addKnob(nuke.Text_Knob("spacer", "", " " * 20))
            ...
            ## layout settings
            self.addKnob(nuke.Text_Knob('spacer_layout', '', ' '))
        ...
        def knobChanged(self, knob):
            ...
            if knob == self.render_engine:
                ...
                ## Safely merge the new engine defaults without clobbering user edits or creating duplicates
                def clean_input(knob_value):
                    return [l for l in multiline_to_list(knob_value) if ":" not in l and "---" not in l and l.strip()]
                ...
                omit_set = clean_input(self.omit_list.value())
                cur_lg_read = clean_input(self.aov_lg_read.value()) if self._lg_read_user_edited else []

                ## Gather anything the user has explicitly placed in another column
                claimed_elsewhere = set(cur_mats + cur_utils + omit_set + cur_lg_read)

                ## Combine existing additions with new engine defaults (ignoring anything claimed elsewhere)
                for l in valid_additional_lighting:
                    if l not in cur_add_light and l not in claimed_elsewhere:
                        cur_add_light.append(l)
            ...
            ## logic for editing the lights read column
            if knob == self.aov_lg_read and self.edit_lg_read.value():
                self._lg_read_user_edited = True
            elif knob == self.edit_lg_read:
                self.aov_lg_read.setEnabled(self.edit_lg_read.value())
        ...
        def update_aov_list(self):
            ...
            ## Create a master set of user overrides so MISC READ never shows duplicates
            user_overrides = set(cur_add_light + cur_mats + cur_utils + list(omit_set) + cur_lg_read)
            ...
            ## column 1: LIGHTS READ
            if not self.edit_lg_read.value() and not self._lg_read_user_edited:
                lg_out = ["LIGHTS READ:", "-" * 20]
                exclusion_set = set(mats + utils + others).union(user_overrides)
                valid_lgs = [l for l in lgs if l not in exclusion_set]
                if valid_lgs:
                    lg_out.extend(valid_lgs)
                lg_out.append("")
                self.aov_lg_read.setValue("\n".join(lg_out))
            ...
            mat_section = "MATERIALS READ:\n" + "-" * 25
            if m_list:
                mat_section += "\n" + "\n".join(m_list)
            other_out.append(mat_section)

            util_section = "\nUTILITIES READ:\n" + "-" * 23
            if u_list:
                util_section += "\n" + "\n".join(u_list)
            other_out.append(util_section)
    ...
    while True:
        ...
        # ## check if a render engine has been selected
        # if p.render_engine.value() == 'None Selected':
        #     nuke.message('Render Engine: None Selected\n\n<center>Please select a render engine before proceeding.')
        #     continue
        #
        # ## check if Generic is selected and warn the user
        # if p.render_engine.value() == 'Generic':
        #     if not nuke.ask('Warning | Render Engine: Generic\n\nMay not categorise AOVs correctly and break the rebuild layout.\n\nProceed?'):
        #         continue ## loops back and re-opens the panel if they click 'No'

        errors = validate_duplicates(p, multiline_to_list)
        if errors:
            nuke.message('\n\n'.join(errors))
    ...
    ## combine the engine defaults with the user's manual additions
    ...
    ## if the user manually edited the LIGHTS READ column, capture that list and use it as the authoritative lightgroup source instead of the regex
    if p._lg_read_user_edited:
        settings['custom_lightgroups'] = clean_input(p.aov_lg_read.value())
    else:
...
def breakout_utilities(node, settings = DEFAULT_SETTINGS):
    ...
    if "alpha" in {u.lower() for u in expected_utilities} and "alpha" not in available_layers_lower:
        if "rgba.alpha" in src_channels:
            ## prevent duplicates if "rgba" was naturally found by get_utilities
            if "rgba" in utilities:
                utilities.remove("rgba") ## check this!
            ## put alpha at the front so it appears first in the utility row
            utilities = ["alpha", "rgba"] + utilities
...
def additive_lightgroups_or_materials(node, mode = 0, settings = DEFAULT_SETTINGS, start_input=None):
    ...
    for lg in lightgroups_or_materials:
        ...
        if mode == 0:
            ...
            if base_suffix:
                ...
                if has_direct and has_indirect:
                    ...
                    target_x, target_y = get_centre_xypos(unpremult_lg)
                    set_centred_xypos(sticky_note, x_pos, target_y - 25)
                    continue

        ## universal guards (apply to both materials and lightgroups)
        if lg_lower in ("ao", "gi") or "raw" in lg_lower or "filter" in lg_lower:
            sticky_label = (
                    "%s AOV not added to B pipe,\n\n"
                    "This AOV will break the basic rebuild.\n\n"
                    "Please use as needed" % lg
            )
            sticky_note = nuke.nodes.StickyNote(
                label=sticky_label,
                tile_color=0x272727ff,
        ...
        if mode == 0 and 'albedo' in lg_lower:
...
def additive_breakout_lightgroups_and_materials(node, settings=DEFAULT_SETTINGS):
    ...
    shuffle_original = nuke.nodes.Shuffle2(inputs=[bpipe_nodes[-1]], label='[value in1] > [value out1]', note_font_color=0xFFFFFFFF, note_font='bold')
    ...
    x_pos -= (x_space * x_dir)
    beauty_dot = nuke.nodes.Dot(inputs = [bpipe_nodes[-1]])
    beauty_dot.setName('beauty_dot', True)
    #beauty_dot['label'].setValue('beauty_dot') ## debug layout
    set_centred_xypos(beauty_dot, x_pos, y_pos)
    ...
    last_shuffle_back_original = None

    ## materials breakout
    if breakout_materials == True and materials:
        mat_branch_dot = nuke.nodes.Dot(inputs=[bpipe_nodes[-1]], )
        mat_branch_dot.setName('mat_branch_dot', True)
        #mat_branch_dot['label'].setValue('mat_branch_dot') ## debug layout
        set_centred_xypos(mat_branch_dot, x_pos, y_pos)
        bpipe_nodes.append(mat_branch_dot)
        x_pos += (x_space * x_dir)

        mat_pipe = additive_lightgroups_or_materials(mat_branch_dot, 0, settings)
        ...
        #mat_dot_bottom['label'].setValue('mat_dot_bottom') ## debug layout
        set_centred_xypos(mat_dot_bottom, x_pos, y_pos)
        bpipe_nodes.append(mat_dot_bottom)

        shuffle_back_original = nuke.nodes.Shuffle2(inputs=[bpipe_nodes[-1]], in1='original', label='original rgb', note_font_color=0xFFFFFFFF, note_font='bold')
        ## track this node for the compare switch
        last_shuffle_back_original = shuffle_back_original
        ...
        y_pos += y_space
        spacer_dot = nuke.nodes.Dot(inputs=[bpipe_nodes[-1]])
        spacer_dot.setName('spacer_dot', True)
        #spacer_dot['label'].setValue('spacer dot!')  ## debug layout
    ...
    if breakout_lightgroups == True and lightgroups:
        ...
        lg_branch_dot.setName('lg_branch_dot', True)
        #lg_branch_dot['label'].setValue('lg_branch_dot') ## debug layout
        set_centred_xypos(lg_branch_dot, x_pos, y_pos)
        bpipe_nodes.append(lg_branch_dot)
        x_pos += (x_space * x_dir)

        lg_pipe = additive_lightgroups_or_materials(lg_branch_dot, 1, settings)
        x_pos = get_centre_xypos(bpipe_nodes[-1])[0]
...
def additive_post_layout_adjustments(y_offset_shuffle=28, y_offset_unpremult=32, y_pad_bottom_dot=50):
    ...
    ## delete spacer_dot if only materials ran
    mat_ran = any('mat_branch_dot' in n.name() for n in nuke.allNodes('Dot'))
    lg_ran = any('lg_branch_dot' in n.name() for n in nuke.allNodes('Dot'))

    if mat_ran and not lg_ran:
        for n in nuke.allNodes('Dot'):
            if n.name().startswith('spacer_dot'):
                nuke.delete(n)
...
def subtractive_lightgroups_or_materials(node, mode = 0, settings = DEFAULT_SETTINGS, start_input=None, upstream_aov_dot=None, upstream_unassigned_dot=None):
    ...
    for i, lg in enumerate(lightgroups_or_materials):
        ...
        ## universal guards (apply to both materials and lightgroups)
        if not skip_bpipe and (lg_lower in ("ao", "gi") or "raw" in lg_lower or "filter" in lg_lower):
            sticky_label = (
                    "%s AOV not added to B pipe,\n\n"
                    "This AOV will break the basic rebuild.\n\n"
                    "Please use as needed" % lg
        ...
        if not skip_bpipe:
            ...
            x_pos -= (x_space * x_dir) * 0.5
            merge_plus = nuke.nodes.Merge2(inputs=[bpipe_nodes[-1], aov_pipe[-1]], operation='plus', output='rgb',
                                           tile_color=MERGE_PLUS_COLOUR)
    ...
    for lg in lightgroups_or_materials:
        unassigned_y_pos += y_space
        unpremult_unassigned_pipe = nuke.nodes.Unpremult(inputs=[unassigned_pipe[-1]], channels=lg)
        set_centred_xypos(unpremult_unassigned_pipe, int(unassigned_x_pos), int(unassigned_y_pos))
        unassigned_pipe.append(unpremult_unassigned_pipe)
    ...
    unassigned_bottom_mat_from_dot = nuke.nodes.Dot(inputs=[unassigned_pipe[-1]])
    unassigned_bottom_mat_from_dot.setName('unassigned_bottom_mat_from_dot', True) ## debug
    #bottom_aov_dot['label'].setValue('bottom_aov_dot')  ## debug layout
    set_centred_xypos(unassigned_bottom_mat_from_dot, int(unassigned_x_pos), bpipe_ypos)
    ...
    if run_qc and ct:
        ...
        if has_negatives:
            ## red / disconnect
            ct['tile_color'].setValue(MERGE_FROM_COLOUR)
            unassigned_bottom_mat_from_dot.setInput(0, None)  ## disconnect unassigned_bottom_mat_from_dot
            msg = "<font color=#eb4c34><b>Negative Values</b></font>\nUnassigned pipe disconnected. Negative values should not be plussed to the B pipe"
            sn = nuke.nodes.StickyNote(label=msg, tile_color=0x272727ff, note_font_color=0xa8a8a8ff, note_font_size=20)
    ...
    ## append the dots to the final list for the return
    unassigned_pipe.append(unassigned_bottom_mat_from_dot)

    merge_from = nuke.nodes.Merge2(inputs = [bpipe_nodes[-1], unassigned_bottom_mat_from_dot], operation ='from', output = 'rgb', tile_color = MERGE_FROM_COLOUR, label = '<i> unassigned from aov', disable = True)
    merge_from.setName('merge_from', True)
    set_centred_xypos(merge_from, bpipe_xpos, bpipe_ypos)
    bpipe_nodes.append(merge_from)

    bpipe_ypos += y_space

    unassigned_bottom_mat_plus_dot = nuke.nodes.Dot(inputs=[unassigned_pipe[-1]])
    unassigned_bottom_mat_plus_dot.setName('unassigned_bottom_mat_plus_dot', True) ## debug
    #unassigned_bottom_mat_plus_dot['label'].setValue('unassigned_bottom_mat_plus_dot')  ## debug layout
    set_centred_xypos(unassigned_bottom_mat_plus_dot, int(unassigned_x_pos), bpipe_ypos)
    unassigned_pipe.append(unassigned_bottom_mat_plus_dot)

    merge_plus = nuke.nodes.Merge2(inputs = [bpipe_nodes[-1], unassigned_bottom_mat_plus_dot], operation ='plus', output = 'rgb', tile_color = MERGE_PLUS_COLOUR, label = '<i> unassigned plus aov', disable = True)
    merge_plus.setName('merge_plus', True)
    set_centred_xypos(merge_plus, bpipe_xpos, bpipe_ypos)
    bpipe_nodes.append(merge_plus)
...
def subtractive_breakout_lightgroups_and_materials(node, settings=DEFAULT_SETTINGS):
    ...
    if breakout_materials == True and materials:
        ...
        mat_pipe, last_mat_aov_dot, last_mat_unassigned_dot = subtractive_lightgroups_or_materials(
            divide_top_dot, 0, settings
        )
    ...
    if breakout_lightgroups == True and lightgroups:
        ...
        lg_pipe, _, _ = subtractive_lightgroups_or_materials(
            original_pipe_end, 1, settings,
            start_input=lg_start_input,
            upstream_aov_dot=last_mat_aov_dot,
            upstream_unassigned_dot=last_mat_unassigned_dot
        )
    ...
    tail_x_pos = int(main_x_pos - ((x_space * x_dir) * 2 if breakout_utilities_enabled else 0))

    ## create a Switch node for A/B comparison
    compare_switch = nuke.nodes.Switch(inputs=[current_bpipe_end, switch_dot], label='Compare Beauty')
    compare_switch["note_font_color"].setValue(int(0xFFFFFFFF))
    compare_switch["note_font"].setValue("bold")
